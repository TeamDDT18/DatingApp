...
export class OrdersService {
  ...
  private currentOrderSource = new ReplaySubject<Order[]>(1);
  currentOrder$ = this.currentOrderSource.asObservable();

  constructor(private http: HttpClient) { }

  getOrders(offset?: number, pageSize?: number, sortField?: string, sortDirection?: string)  {
    return this.http.get<Order[]>(this.baseUrl + 'table/orders').pipe(
      map((response:Order[]) => {
        const table = response;
        if (table){
          localStorage.setItem('table',JSON.stringify(table));
          this.currentOrderSource.next(table);
        }
